...
/*   Updated: 2025/07/16 11:44:00 by anagarri@st      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	color_a(double t, double i, double iterations)
{
	...
	if (i == iterations)
		return (0x000000FF);
	t = pow(t, 0.3);
	rgba |= ((unsigned int)(sin(t * 6.28318) * 127 + 128) << 24);
	rgba |= ((unsigned int)(sin(t * 6.28318 + 2.094) * 127 + 128) << 16);
...
unsigned int	color_c(double t, double i, double iterations)
{
	...
	if (i == iterations)
		return (0x000000FF);
	rgba = 0;
	rgba |= ((unsigned int)(9 * (1 - t) * t * t * t * 255) << 24);
	rgba |= ((unsigned int)(15 * (1 - t) * (1 - t) * t * t * 255) << 16);
	rgba |= ((unsigned int)(8.5 * (1 - t) * (1 - t) * t * 255) << 8);
	rgba |= 255;
	return (rgba);
}
